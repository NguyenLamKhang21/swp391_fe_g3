...
const OrderCard = ({ order, storeName, onRefresh }) => {
  ...
  const handleCreateDebtRecord = async () => {
    ...
    if (!amount || amount <= 0) { toast.warn("Vui lòng nhập số tiền nợ hợp lệ."); return; }
    try {
      setActionLoading(true);
      await createDebtRecord(order.storeId, amount);
      toast.success(`Đã tạo phiếu nợ ${amount.toLocaleString()} VND cho store ${storeName(order.storeId)}.`);
      setDebtAmount("");
      setShowDebtForm(false);
      onRefresh();
    } catch (err) {
      toast.error(err?.response?.data?.message ?? "Tạo phiếu nợ thất bại.");
    } finally { setActionLoading(false); }
  };
    ...
    <div>
      {detailLoading ? (
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {/* ── Section 7: Tạo phiếu nợ thủ công (chỉ cho CASH) ── */}
          {order.paymentMethod === "CASH" && (
          <div className="border border-border rounded-lg p-4 space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                <DollarSign className="w-3.5 h-3.5 text-primary" />
                Tạo phiếu nợ cho Store
              </h4>
              {!showDebtForm && (
                <button
            ...
            {showDebtForm && (
              <div className="space-y-2 bg-amber-50 border border-amber-200 rounded-lg p-3">
                <p className="text-xs text-amber-700">
                  Tạo phiếu nợ cho <strong>{storeName(order.storeId)}</strong>
                </p>
                <div className="flex items-end gap-2">
                  <div className="flex-1">
                    <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Số tiền nợ (VND)</label>
                    <input
                      type="number"
                      min="1"
                      ...
                      className="um-input text-sm w-full mt-1"
                    />
                  </div>
                  <button
                    onClick={handleCreateDebtRecord}
                    disabled={actionLoading || !debtAmount}
                    className="flex items-center gap-2 px-4 py-2 rounded-lg bg-amber-600 text-white text-sm font-semibold hover:bg-amber-700 transition-colors disabled:opacity-60"
                  >
                    {actionLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <DollarSign className="w-4 h-4" />}
                    Xác nhận
                  </button>
                  <button
                    onClick={() => { setShowDebtForm(false); setDebtAmount(""); }}
                    className="px-4 py-2 rounded-lg border border-border text-sm font-medium hover:bg-muted transition-colors"
                  >
                    Hủy
                  </button>
